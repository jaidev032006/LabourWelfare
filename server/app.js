...
apiRouter.get('/health', (_, res) => res.json({ ok: true, mode: process.env.SUPABASE_URL ? 'supabase-ready-demo-data' : 'demo-mode' }));
apiRouter.get('/categories', (_, res) => res.json({ categories, services }));
...
apiRouter.post('/auth/login', async (req, res) => {
  ...
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@labourwelfare.local';
...
apiRouter.get('/me', auth(), (req, res) => res.json({ user: publicUser(findUser(req.user.id)) }));
apiRouter.get('/workers', auth(['customer', 'admin']), (req, res) => res.json({ workers: matchWorkers(req.query) }));

apiRouter.post('/ai/analyze', auth(['customer']), upload.single('image'), async (req, res) => {
  try {
...
apiRouter.get('/bookings', auth(['customer', 'worker', 'admin']), (req, res) => {
...
apiRouter.post('/bookings', auth(['customer']), (req, res) => {
...
apiRouter.post('/bookings/:id/respond', auth(['worker']), (req, res) => {
...
apiRouter.post('/bookings/:id/status', auth(['worker']), (req, res) => {
  const booking = bookings.find(item => item.id === req.params.id && item.workerId === req.user.id);
...
apiRouter.get('/workers/:id/reviews', auth(['customer', 'worker', 'admin']), (req, res) => {
...
// Support both /api/categories and /categories routing
